...
void gui::run() {
    ...
    while (ui.active()) {
        ...
        ImGui::Begin("Yay Window");  // Pass a pointer to our bool variable (the window will have a closing
                                     // button that will clear the bool when clicked)
        ImGui::Text("%s", std::to_string(display_data_.last_executed_event_ts_).c_str());
