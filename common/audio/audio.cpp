...
Audio::Audio() {
  ...
  sdl_audio_device_id_ =
    SDL_OpenAudioDevice(nullptr, 0, &desired_spec, &sdl_audio_spec_, 0);
...
void Audio::playSound(Audio::Wav const& wav, int volume) {
  ...
  SDL_MixAudioFormat(&buffer_.front(),
                     wav.buffer,
                     32784,
                     wav.buffer_size,
                     volume);
...
Audio::Wav::Wav(std::string const& filename) {
  auto* spec =
    SDL_LoadWAV(filename.c_str(), &audio_spec, &buffer, &buffer_size);
  if (spec == nullptr) {
